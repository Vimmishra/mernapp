...
const HorrorMovies = () => {
  ...
  const getHorrorMovies = async () => {
    try {
      const res = await axiosInstance.get("/api/videos/horrorMovies");
      setMovies(res.data.data || []);
    } catch (err) {
      console.log("Error fetching horror movies:", err);
    }
  };
...
const userId = user.id;
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-semibold mb-6 text-white">Horror Movies</h2>
      ...
      <div className="flex overflow-x-auto space-x-6 scrollbar-hide px-1">
        ...
        {/* View More Card */}
        <div
          onClick={() => navigate("/view-more/horror")}
        ...
        >
          <div className="h-full bg-gradient-to-r from-zinc-800 to-zinc-900 rounded-xl flex flex-col justify-center items-center text-white shadow-lg hover:shadow-2xl transition duration-300 transform hover:-translate-y-1 hover:scale-105">
            ...
            <span className="text-sm text-gray-400 mt-1">Explore all horror movies</span>
          </div>
        </div>
      </div>
    </div>
